...
class RandomOrganismCreator(object):
    ...
    def create_organism(self, id_generator, composition_space, constraints,
        ...
        random_org = Organism(random_cell, id_generator, self.name,
                              composition_space)
        logstr='Random organism creator making organism {} '.format(
            random_org.id)
        ...
        if self.files[self.num_made - 1].endswith('.cif') or self.files[
            ...
            try:
                ...
                new_org = Organism(new_cell, id_generator, self.name,
                                   composition_space)
                logstr = 'Making organism {} from file: {} '.format(
                    new_org.id, self.files[self.num_made - 1])
                log_writer.write_data(logstr)
                self.update_status()
                return new_org
            except:
                logstr='Error reading structure from file: {} '.format(
                    self.files[self.num_made - 1])
                log_writer.write_data(logstr)
                self.update_status()
                return None
        else:
            logstr=f'File {self.files[self.num_made - 1]} has invalid extension - ' + \
                   f'file must end with .cif or begin with POSCAR '
            log_writer.write_data(logstr)
            self.update_status()
            return None

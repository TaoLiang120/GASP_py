...
class Mating(object):
    ...
    def do_variation(self, pool, random, geometry, constraints, id_generator,
        ...
        offspring = Organism(offspring_cell, id_generator, self.name,
                             composition_space)
        logstr = f"Creating offspring organism {offspring.id} from the mating variations with parent1 {parent1.id} and parent2 {parent2.id}."
    ...
    def make_offspring_cell(self, parent_cell_1, parent_cell_2, geometry,
        ...
        offspring_lattice = Lattice.from_parameters(offspring_lengths[0], offspring_lengths[1], offspring_lengths[2],
                                                    offspring_angles[0], offspring_angles[1], offspring_angles[2])
...
class StructureMut(object):
    ...
    def do_variation(self, pool, random, geometry, constraints, id_generator,
        ...
        logstr=f"Creating offspring organism {offspring.id} from the structure mutation variation with the parent {parent_org.id}."
        log_writer.write_data(logstr)
...
class NumAtomsMut(object):
    ...
    def do_variation(self, pool, random, geometry, constraints, id_generator,
        ...
        offspring = Organism(cell, id_generator, self.name, composition_space)
        logstr=f"Creating offspring organism {offspring.id} from the number of atoms mutation variation with the parent {parent_org.id}."
        log_writer.write_data(logstr)
        return offspring
...
class Permutation(object):
    ...
    def do_variation(self, pool, random, geometry, constraints, id_generator,
        ...
        offspring = Organism(cell, id_generator, self.name, composition_space)
        logstr=f"Creating offspring organism {offspring.id} from the permutation variation with the parent {parent_org.id}."
        log_writer.write_data(logstr)

...
class Cell(Structure):
    ...
    def modify_lattice(self, thislattice):
        if isinstance(thislattice, Lattice):
            self.lattice = Lattice(thislattice.matrix)
        else:
            self.lattice = Lattice(thislattice)
        ...
        while True:
            ...
            while num_tries < max_num_tries:
                ...
                if developer.develop(
                        ...
                        pool, log_writer) and (redundancy_guard.check_redundancy(
                    offspring, whole_pop, geometry, log_writer) is None):
                    return offspring
                else:
...
class DataWriter(object):
    ...
    def __init__(self, file_path, composition_space, RESTART=False):
        """
        Makes a DataWriter.
...
class LogWriter(object):
    ...
    def __init__(self, file_path, Screen=True, Log=True, RESTART=False):
        """
        Makes a DataWriter.

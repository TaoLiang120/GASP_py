...
class Developer(object):
    ...
    def develop(self, organism, composition_space, constraints, geometry,
        ...
        if not self.satisfies_num_atoms_constraints(organism, constraints, log_writer):
            return False

        # check if the organism is is the composition space
        if not self.is_in_composition_space(organism, composition_space,
                                            constraints, pool, log_writer):
            return False

        # optionally do Niggli cell reduction
        if self.niggli:
            if not self.niggli_reduction(organism, geometry, constraints, log_writer):
                return False
        ...
        if self.scale_density and len(
            ...
            if not self.scale_volume(organism, composition_space, pool, log_writer):
                return False

        # check the lattice length and angle constraints
        if not self.satisfies_lattice_constraints(organism, constraints, log_writer):
            return False

        # check the per-species minimum interatomic distance constraints
        if not self.satisfies_mids_constraints(organism, constraints, log_writer):
            return False

        # check any geometry-specific constraints
        if not self.satisfies_geometry_constraints(organism, geometry, log_writer):
            return False

        return True

    def satisfies_num_atoms_constraints(self, organism, constraints, log_writer):
        """
        ...
        if len(organism.cell.sites) > constraints.max_num_atoms:
            logstr = "Organism {} failed max number of atoms constraint ".format(
                organism.id)
        ...
        if len(organism.cell.sites) < constraints.min_num_atoms:
            logstr = "Organism {} failed min number of atoms constraint ".format(
                organism.id)
            log_writer.write_data(logstr)
            return False
        return True
    ...
    def is_in_composition_space_epa(self, organism, composition_space, log_writer):
        """
        ...
        if not reduced_composition.almost_equals(org_reduced_composition):
            logstr = "Organism {} has incorrect composition ".format(organism.id)
            log_writer.write_data(logstr)
            return False
        return True
    ...
    def is_in_composition_space_pd(self, organism, composition_space,
        ...
        if len(composition_checker.transform_entries(
            ...
            logstr = 'Organism {} lies outside the composition space '.format(
                organism.id)
            log_writer.write_data(logstr)
            return False
        ...
        if not constraints.allow_endpoints and len(pool.to_list()) > 0:
            for endpoint in composition_space.endpoints:
                if endpoint.almost_equals(
                    ...
                    logstr = 'Organism {} is at a composition space endpoint '.format(organism.id)
                    log_writer.write_data(logstr)
                    return False
        return True

    def niggli_reduction(self, organism, geometry, constraints, log_writer):
        """
        ...
        if geometry.shape == 'bulk':
            if not organism.cell.reduce_cell():
                logstr = 'Niggli cell reduction failed on organism {} during development '.format(organism.id)
                log_writer.write_data(logstr)
                return False
        elif geometry.shape == 'sheet':
            if not organism.cell.reduce_sheet_cell(geometry, constraints):
                logstr = '2D Niggli cell reduction failed on organism {} during development '.format(organism.id)
                log_writer.write_data(logstr)
                return False
    ...
    def scale_volume(self, organism, composition_space, pool, log_writer):
        """
        ...
        if composition_space.objective_function == 'epa':
            return self.scale_volume_epa(organism, pool, log_writer)
        elif composition_space.objective_function == 'pd':
            return self.scale_volume_pd(organism, composition_space, pool, log_writer)

    def scale_volume_epa(self, organism, pool, log_writer):
        """
        Returns a boolean indicating whether volume scaling did not fail.
        ...
        with warnings.catch_warnings():
            ...
            if str(organism.cell.lattice.a) == 'nan' or \
                ...
                logstr = 'Volume scaling failed on organism {} during development '.format(organism.id)
                log_writer.write_data(logstr)
                return False
        return True

    def scale_volume_pd(self, organism, composition_space, pool, log_writer):
        """
        Returns a boolean indicating whether volume scaling did not fail.
    ...
    def satisfies_lattice_constraints(self, organism, constraints, log_writer):
        """
        Returns a boolean indicating whether the organism satisfies the
        ...
        for length in lengths:
            if length > constraints.max_lattice_length:
                logstr = 'Organism {} failed max lattice length constraint '.format(organism.id)
                log_writer.write_data(logstr)
                return False
            elif length < constraints.min_lattice_length:
                logstr = 'Organism {} failed min lattice length constraint '.format(organism.id)
                log_writer.write_data(logstr)
                return False
        ...
        for angle in angles:
            if angle > constraints.max_lattice_angle:
                logstr = 'Organism {} failed max lattice angle constraint '.format(organism.id)
                log_writer.write_data(logstr)
                return False
            elif angle < constraints.min_lattice_angle:
                logstr = 'Organism {} failed min lattice angle constraint '.format(organism.id)
                log_writer.write_data(logstr)
                return False
        return True

    def satisfies_mids_constraints(self, organism, constraints, log_writer):
        """
        Returns a boolean indicating whether the organism satisfies the
            ...
            for species_symbol in species_symbols:
                ...
                for neighbor in neighbors:
                    if neighbor[0].specie.symbol == species_symbol:
                        logstr = f'Organism {organism.id} failed minimum interatomic distance constraint '
                        log_writer.write_data(logstr)
                        return False
        return True

    def satisfies_geometry_constraints(self, organism, geometry, log_writer):
        """
        Returns a boolean indicating whether the organism satisfies the
        ...
        if geometry.get_size(organism.cell) > geometry.max_size:
            logstr = "Organism {} failed max size constraint ".format(
                organism.id)
            log_writer.write_data(logstr)
            return False
        ...
        if geometry.get_size(organism.cell) < geometry.min_size:
            logstr = "Organism {} failed min size constraint ".format(
                organism.id)
            log_writer.write_data(logstr)
            return False
        return True
...
class RedundancyGuard(object):
    ...
    def check_redundancy(self, new_organism, orgs_list, geometry, log_writer):
        '''
            ...
            for organism in orgs_list:
                if new_organism.id != organism.id:  # just in case
                    ...
                    if self.check_structures(new_organism, organism, geometry):
                        logstr = f'Organism {new_organism.id} failed structural redundancy - ' + \
                                 f'looks like organism {organism.id} '
                        log_writer.write_data(logstr)
                        return organism
            ...
            for organism in orgs_list:
                if new_organism.id != organism.id and organism.epa is not None:
                    ...
                    if abs(new_organism.epa - organism.epa) < self.epa_diff:
                        logstr = f'Organism {new_organism.id} failed energy per atom redundancy - ' + \
                                 f'looks like organism {organism.id} '
                        log_writer.write_data(logstr)
                        return organism
        return None

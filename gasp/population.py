...
class InitialPopulation:
    ...
    def add_organism(self, organism_to_add, composition_space, log_writer):
        """
        ...
        organism_to_add.cell.to(fmt="poscar", filename=os.getcwd() + "/POSCAR." + str(organism_to_add.id))
        logstr = 'Adding organism {} to the initial population'.format(
            organism_to_add.id)
        log_writer.write_data(logstr)
        self.initial_population.append(organism_to_add)
        organism_to_add.is_active = True

    def replace_organism(self, old_org, new_org, composition_space, log_writer):
        """
        ...
        new_org.cell.to(fmt='poscar', filename=os.getcwd() + '/POSCAR.' +
                                               str(new_org.id))
        logstr = f'Replacing organism {old_org.id} with organism {new_org.id} in the initial population'
        log_writer.write_data(logstr)
...
class Pool(object):
    ...
    def __init__(self, pool_params, composition_space, run_dir_name):
        ...
        self.default_interval_to_print = 50
        ...
        if pool_params in (None, 'default'):
            ...
            self.interval_to_print = self.default_interval_to_print
        else:
            ...
            if 'interval_to_print' not in pool_params:
                self.interval_to_print = self.default_interval_to_print
            elif pool_params['interval_to_print'] in (None, 'default'):
                self.interval_to_print = self.default_interval_to_print
            elif not isinstance(pool_params['interval_to_print'], int):
                self.interval_to_print = self.default_interval_to_print
            else:
                self.interval_to_print = pool_params['interval_to_print']
    ...
    def add_initial_population(self, initial_population, composition_space, log_writer):
        """
        ...
        logstr = 'Populating the pool with the initial population...'
        log_writer.write_data(logstr)
        ...
        logstr = 'Summary of the initial population: '

        for organism in organisms_list:
            logstr += '\n' + f'Organism {organism.id} has value {organism.value} and ' + \
                      f'fitness {organism.fitness} and selection probability {organism.selection_prob} '
        log_writer.write_data(logstr)
    ...
    def add_organism(self, organism_to_add, composition_space, log_writer):
        ...
        logstr = 'Adding organism {} to the pool '.format(organism_to_add.id)
        log_writer.write_data(logstr)
    ...
    def replace_organism(self, old_org, new_org, composition_space, log_writer):
        ...
        logstr = 'Replacing organism {} with organism {} in the pool '.format(
    ...
    def print_summary(self, composition_space, num_calcs, log_writer):
        """
        ...
        Args:
            composition_space: the CompositionSpace of the search
        """
        if num_calcs % self.interval_to_print == 0:
            pool_list = self.to_list()
            pool_list.sort(key=lambda x: x.value, reverse=False)
            logstr = 'Summary of the pool:'
            for organism in pool_list:
                logstr += '\n' + f'Organism {organism.id} has value {organism.value} and ' + \
                          f'fitness {organism.fitness} and selection probability {organism.selection_prob} '
            log_writer.write_data(logstr)
